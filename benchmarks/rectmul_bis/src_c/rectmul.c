...
add_matrix(block *T, long ot, block *R, long or_, long x, long y)
{
  ...
  if ((x+y)==2) {
    ...
    if (x>y) {
      flops0 = add_matrix(T,ot,R,or_,x/2,y);
      flops1 = add_matrix(T+(x/2)*ot,ot,R+(x/2)*or_,or_,(x+1)/2,y);
    } 
    else {
      flops0 = add_matrix(T,ot,R,or_,x,y/2);
      flops1 = add_matrix(T+(y/2),ot,R+(y/2),or_,x,(y+1)/2);
    }
...
multiply_matrix(block *A, long oa, block *B, long ob, long x, long y, long z, block *R, long or_, int add)
{
  if ((x+y+z) == 3) {
    ...
    if ((x>=y) && (x>=z)) {  
      flops0 = multiply_matrix(A,oa,B,ob,x/2,y,z,R,or_,add);
      flops1 = multiply_matrix(A+(x/2)*oa,oa,B,ob,(x+1)/2,y,z,R+(x/2)*or_,or_,add);
    } 
    else {
      if ((y>x) && (y>z)){
            flops0 = multiply_matrix(A+(y/2),oa,B+(y/2)*ob,ob,x,(y+1)/2,z,R,or_,add);

            if (/*SYNCHED*/1) {
	      flops1 = multiply_matrix(A,oa,B,ob,x,y/2,z,R,or_,1);
            } 
	    else {
	      block *tmp = (block *)alloca(x*z*sizeof(block));

	      flops0 = multiply_matrix(A,oa,B,ob,x,y/2,z,tmp,z,0);
	      
	      flops1 = add_matrix(tmp,z,R,or_,x,z);
            }
      }
      else {
	flops0 = multiply_matrix(A,oa,B,ob,x,y,z/2,R,or_,add);
	flops1 = multiply_matrix(A,oa,B+(z/2),ob,x,y,(z+1)/2,(R+(z/2)),or_,add);
      } 
    }
...
run(long x, long y, long z, int check)
{
  ...
  if (check) {
    check = check_matrix(R, x, z, z, y*BLOCK_EDGE);
  }
