...
multiply_matrix(block *A, long oa, block *B, long ob, long x, long y, long z, block *R, long or_, int add)
{
	...
	if ((x>=y) && (x>=z)) {  
	    spawn(multiply_matrix, flops1, A,oa,B,ob,x/2,y,z,R,or_,add);
	    spawn(multiply_matrix, flops2, A+(x/2)*oa,oa,B,ob,(x+1)/2,y,z,R+(x/2)*or_,or_,add);
	} 
	else {
	    if ((y>x) && (y>z)){
		spawn(multiply_matrix, flops1, A+(y/2),oa,B+(y/2)*ob,ob,x,(y+1)/2,z,R,or_,add);

		if (SYNCHED() || true) {
		    ssync();
		    spawn(multiply_matrix, flops2, A,oa,B,ob,x,y/2,z,R,or_,1);
		} else {
		    block *tmp = (block *)/*Cilk_*/alloca(x*z*sizeof(block));

		    spawn(multiply_matrix, flops2, A,oa,B,ob,x,y/2,z,tmp,z,0);
		    ssync();
		...
		spawn(multiply_matrix, flops1, A,oa,B,ob,x,y,z/2,R,or_,add);
		spawn(multiply_matrix, flops2, A,oa,B+(z/2),ob,x,y,(z+1)/2,(R+(z/2)),or_,add);
	    } 
	}
...
int 
run_rectmul(long x, long y, long z, int check)
{
  block *A, *B, *R;
  long long flops;
  double f;
  ...
  run(init_matrix, R, x, z, z, 0.0);
       // sync;
  
  /* Timing. "Start" timers */
  pp_time_start( &tm );
  
  flops = run(multiply_matrix,A,y,B,z,x,y,z,R,z,0);
...
main(int argc, char *argv[])
{
  ...
  t = run_rectmul((long)x,(long)y,(long)z,(int)check);
