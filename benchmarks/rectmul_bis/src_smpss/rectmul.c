...
#pragma css task input(A, B) inout(R)
void
mult_add_block_dep(block * A, block * B, block R)
{
    mult_add_block( A, B, (block *)R );
}
...
add_matrix(block *T, long ot, block *R, long or_, long x, long y)
{
  if ((x+y)==2) {
    ...
    if (x>y) {
	add_matrix( T,ot,R,or_,x/2,y);
	add_matrix( T+(x/2)*ot,ot,R+(x/2)*or_,or_,(x+1)/2,y);
    } 
    else {
	add_matrix( T,ot,R,or_,x,y/2);
	add_matrix( T+(y/2),ot,R+(y/2),or_,x,(y+1)/2);
    }
...
long long 
multiply_matrix_ooo(block *A, long oa, block *B, long ob, long x, long y, long z, block *R, long or_, int add)
{
    if ((x+y+z) == 3) {
	if (add)
	...
	for( unsigned j=0; j < y; ++j ) {
		...
		for( unsigned k=0; k < z; ++k ) {
		    mult_add_block_dep( (A+i*oa+j), (B+j*ob+k), *(R+i*or_+k));
		}
	    }
	}
	return sflops;
...
#pragma css start
  ...
  flops = multiply_matrix_ooo(A,y,B,z,x,y,z,R,z,0);
...
#pragma css finish
  ...
  if (check) {
      check = check_matrix( R, x, z, z, (double)(y*BLOCK_EDGE));
  }
