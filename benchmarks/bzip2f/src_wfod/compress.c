...
// C++ functions because they have an output writer dependency
#ifdef BZLIB_WFO
...
void VPrintf0Fn( obj::inoutdep<int> seq, const char * zf ) {
    fprintf( stderr, zf );
}
template<typename T1>
void VPrintf1Fn( obj::inoutdep<int> seq, const char * zf, T1 za1 ) {
    fprintf( stderr, zf, za1 );
}
template<typename T1, typename T2>
void VPrintf2Fn( obj::inoutdep<int> seq, const char * zf, T1 za1, T2 za2 ) {
    fprintf( stderr, zf, za1, za2 );
}
template<typename T1, typename T2, typename T3>
void VPrintf3Fn( obj::inoutdep<int> seq, const char * zf, T1 za1, T2 za2, T3 za3 ) {
    fprintf( stderr, zf, za1, za2, za3 );
}

template<typename T1, typename T2, typename T3, typename T4>
void VPrintf4Fn( obj::inoutdep<int> seq, const char * zf, T1 za1, T2 za2, T3 za3,
	       T4 za4 ) {
    fprintf( stderr, zf, za1, za2, za3, za4 );
}
template<typename T1, typename T2, typename T3, typename T4, typename T5>
void VPrintf5Fn( obj::inoutdep<int> seq, const char * zf, T1 za1, T2 za2, T3 za3,
	       T4 za4, T5 za5 ) {
    fprintf( stderr, zf, za1, za2, za3, za4, za5 );
}

#define VPrintf0(seq,zf) spawn(VPrintf0Fn,seq,zf)
#define VPrintf1(seq,zf,za) do { spawn(VPrintf1Fn<decltype(za)>,seq,(const char *)zf,za); } while(0)
#define VPrintf2(seq,zf,za1,za2) spawn(VPrintf2Fn<decltype(za1),decltype(za2)>,seq,zf,za1,za2)
#define VPrintf3(seq,zf,za1,za2,za3) spawn(VPrintf3Fn<decltype(za1),decltype(za2),decltype(za3)>,seq,zf,za1,za2,za3)
#define VPrintf4(seq,zf,za1,za2,za3,za4) spawn(VPrintf4Fn<decltype(za1),decltype(za2),decltype(za3),decltype(za4)>,seq,zf,za1,za2,za3,za4)
#define VPrintf5(seq,zf,za1,za2,za3,za4,za5) spawn(VPrintf5Fn<decltype(za1),decltype(za2),decltype(za3),decltype(za4),decltype(za5)>,seq,zf,za1,za2,za3,za4,za5)

#else
namespace obj {
template<typename T>
struct object_t { T value; };
}
#endif

static obj::object_t<int> verbosity_output;
static void sendMTFValues ( EState* s, obj::delegated_inoutdep<int> v );
...
static
void sendMTFValues ( EState* s, obj::delegated_inoutdep<int> verbosity_seq )
{
   ...
   if (s->verbosity >= 3)
       VPrintf3( (obj::inoutdep<int>)verbosity_seq,
   ...
   { 
      ...
      while (nPart > 0) {
         ...
         if (s->verbosity >= 3)
	     VPrintf5( (obj::inoutdep<int>)verbosity_seq,
		     "      initial group %d, [%d .. %d], "
                      "has %d syms (%4.1f%%)\n",
                      nPart, gs, ge, aFreq, 
...
#           undef BZ_ITUR
      ...
      if (s->verbosity >= 3) {
	  VPrintf2( (obj::inoutdep<int>)verbosity_seq,
		    "      pass %d: size is %d, grp uses are ", 
                   iter+1, totc/8 );
         for (t = 0; t < nGroups; t++)
	     VPrintf1( (obj::inoutdep<int>)verbosity_seq, "%d ", (int)fave[t] );
         VPrintf0( (obj::inoutdep<int>)verbosity_seq, "\n" );
      }
   ...
   { 
      ...
      if (s->verbosity >= 3) 
	  VPrintf1( (obj::inoutdep<int>)verbosity_seq, "      bytes: mapping %d, ", s->numZ-nBytes );
   }
   ...
   if (s->verbosity >= 3)
       VPrintf1( (obj::inoutdep<int>)verbosity_seq, "selectors %d, ", s->numZ-nBytes );
   ...
   if (s->verbosity >= 3)
       VPrintf1( (obj::inoutdep<int>)verbosity_seq, "code lengths %d, ", s->numZ-nBytes );
...
#           undef BZ_ITAH
   ...
   if (s->verbosity >= 3)
       VPrintf1( (obj::inoutdep<int>)verbosity_seq, "codes %d\n", s->numZ-nBytes );
...
/*---------------------------------------------------*/
void BZ2_compressBlockInternal ( EState* s, Bool is_last_block,
			 obj::delegated_inoutdep<int> verbosity_seq )
{
   if (s->nblock > 0) {
      ...
      if (s->verbosity >= 2)
	  VPrintf4( (obj::inoutdep<int>)verbosity_seq,
		    "    block %d: crc = 0x%08x, "
   ...
   if (s->nblock > 0) {
      ...
      bsW ( s, 24, s->origPtr );
      generateMTFValues ( s );
      spawn( sendMTFValues, s, verbosity_seq );
      ssync();
   }
   ...
   if (is_last_block) {
      ...
      if (s->verbosity >= 2)
	  VPrintf1( (obj::inoutdep<int>)verbosity_seq,
...
void BZ2_compressBlock ( EState* s, Bool is_last_block ) {
    obj::object_t<int> verbosity_seq;
    spawn( BZ2_compressBlockInternal, s, is_last_block,
	   (obj::delegated_inoutdep<int>) verbosity_seq );
    // ssync( verbosity_seq );
    ssync();
}
...
static void report(long long in, long long out,
		   obj::inoutdep<int> verbosity_seq )
{
    if (in == 0) {
                      ...
                      int workFactor) throw()
{
...

    {
        ...
        if (verbosity)
            spawn( report,bytes_in, ((VIEW(output).total + 7) / 8),
		   (obj::inoutdep<int>)verbosity_seq);
    }
                         ...
                         int workFactor) throw()
{
...

    {
        ...
        if (verbosity)
            spawn( report, size, ((VIEW(output).total + 7) / 8),
		   (obj::inoutdep<int>)verbosity_seq);
    }
                   ...
                   const bitblock overflow)
{
    ...
    assert(carry.numbits >= 0 && carry.numbits < 8);
...
static void BZ2_compressBlockWFO(EState *s, obj::outdep<int>,
				 obj::delegated_inoutdep<int> verbosity_seq );
...
void finishCompressWFO_leaf(writer * output, unsigned char * trailer)
{
    /* This write destroys the cumulative checksum CRC,
       which was already written. */           
		       ...
		       obj::delegated_inoutdep<int> verbosity_seq)
{
    ...
    leaf_call( finishCompressWFO_leaf, output, trailer );

    if (verbosity)
	spawn( report, (long long)bytes_in,
	       (long long)((output->total + 7) / 8),
	       (obj::inoutdep<int>)verbosity_seq);
                      ...
                      int workFactor) throw()
{
	...
	obj::object_t<int> dep_cw;
	obj::object_t<int> dep_w;
        ...
        while (!feof(in)) {
            ...
            /* TODO: conditional sync here if too out of order */
            if (s->nblock) {
                spawn( BZ2_compressBlockWFO, s, (obj::outdep<int>)dep_cw,
		       (obj::delegated_inoutdep<int>)verbosity_seq );

		/* Last pipeline stage */
		spawn( BZ2_writeBlockWFO, s, &output, (obj::indep<int>)dep_cw,
		       (obj::inoutdep<int>)dep_w );
            } else {
                free(s->arr1);
                free(s->arr2);
                free(s->ftab);
	...
	spawn( finishCompressWFO, verbosity, (int)bytes_in, &output,
	       (obj::delegated_inoutdep<int>)verbosity_seq );
        ssync();
...
static void
BZ2_compressBlockWFO(EState *s, obj::outdep<int>, obj::delegated_inoutdep<int> verbosity_seq)
{
    BZ_FINALISE_CRC(s->blockCRC);
    s->numZ = 0; /* flushed after every block */
    ...
    if (s->verbosity >= 2)
        VPrintf2((obj::inoutdep<int>)verbosity_seq,
		    "    block crc = 0x%08x, size = %d\n", s->blockCRC, s->nblock);

    leaf_call(BZ2_blockSort,s);
    s->zbits = (UChar*) (&((UChar*)s->arr2)[s->nblock]);
    ...
    bsW(s,1,0);
    bsW(s, 24, s->origPtr);
    leaf_call(generateMTFValues,s);
    spawn( sendMTFValues, s, verbosity_seq );

    // ssync( verbosity_seq );
    ssync();
    leaf_call(bsFinishWriteWFO,s); // after sendMTFValues finishes
}
...
BZ2_writeBlockWFO(EState *s, writer *output_,
		  obj::indep<int>, obj::inoutdep<int> ) {
    // errs() << " ** write WFO " << s << "\n";
    leaf_call( BZ2_writeBlockWFO_leaf, s, output_ );
    // errs() << " ** write WFO " << s << " done\n";
}
		      ...
		      obj::delegated_inoutdep<int> verbosity_seq)
{
    ...
    // errs() << " ** compress WFOr " << s << "\n";

    length = leaf_call(run_length_encode,s->block, &crc, s->inUse, begin, end);
    s->nblock = length;
    s->blockCRC = crc;
                         ...
                         int workFactor ) throw()
{
    ...
    if (addrv == MAP_FAILED)
    {
        if (verbosity)
            VPrintf1((obj::inoutdep<int>)verbosity_seq,
		    "Unable to map input as stream (%s)\n    ", strerror(errno));
        /*perror ("mmap");*/
        return BZ_STREAM_ERROR;
    }
    unsigned char *addr = (unsigned char *)addrv;
	...
	obj::object_t<int> dep_cw, dep_w;
	    ...
	    // errs() << " ** Init " << s << "\n";
            ...
            spawn( BZ2_compressBlockWFOr, s, base, last,
		   (obj::outdep<int>)dep_cw,
		   (obj::delegated_inoutdep<int>) verbosity_seq );
            spawn( BZ2_writeBlockWFO, s, &output, (obj::indep<int>)dep_cw,
		   (obj::inoutdep<int>)dep_w  );
            base = last;
	...
	spawn( finishCompressWFO, verbosity, (int)size, &output,
	       (obj::delegated_inoutdep<int>)verbosity_seq );
	ssync();
        ...
        if (verbosity)
	    spawn( report, bytes_in, ((output->total + 7) / 8),
		   (obj::inoutdep<int>)verbosity_seq);
#endif
    }
