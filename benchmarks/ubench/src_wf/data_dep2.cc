...
void taskWithArg( DepTy<T> arg1, DepTy<T> arg2) {
// Do something ...
    int x = 0;
    int fibo=g_maxfibo;
    // for( int i=0; i<g_maxfibo; ++i )
...
void par_region( unsigned int niters, object_t<T> * obj1, object_t<T> * obj2 ) {
    for( unsigned int j=0; j < niters; ++j )
	spawn( taskWithArg<M, DepTy>, (DepTy<M>)*obj1, (DepTy<M>)*obj2 );
    ssync();
}

template<typename M, typename T, template<typename U> class DepTy>
void par_region2( unsigned int niters, unsigned int nobj, object_t<T> * obj1, object_t<T> * obj2) {
    unsigned int i = 0;
    for( unsigned int j=0; j < niters; ++j ) {
	spawn( taskWithArg<M, DepTy>, (DepTy<M>)obj1[i], (DepTy<M>)obj2[i]);
	if( ++i == nobj )
	    i = 0;
    }
    ssync();
...
void par_region3( unsigned int niters, unsigned int nobj,
	...
	// errs() << "j=" << j << " i=" << i << "\n";
	if( i < batch1 )
	    spawn( taskWithArg<T, indep>, (indep<T>)*obj1, (indep<T>)*obj2);
	else
	    spawn( taskWithArg<M, DepTy>, (DepTy<M>)*obj1, (DepTy<M>)*obj2);
...
template<typename T, template<typename U> class DepTy, typename M = T>
double experiment( unsigned int niters, unsigned int nobj, unsigned int batch1,
		   unsigned int batch2, object_t<T> * obj1, object_t<T> * obj2)
    __attribute__((always_inline));
...
double experiment( unsigned int niters, unsigned int nobj,
    ...
    get_time( &start );
    if( batch1+batch2 > 0 )
	run( par_region3<M, T, DepTy>, niters, nobj, batch1, batch2, obj1, obj2);
    else if( nobj > 1 )
	run( par_region2<M, T, DepTy>, niters, nobj, obj1, obj2 );
    else
	run( par_region<M, T, DepTy>, niters, obj1, obj2 );
    get_time( &end );
...
int main( int argc, char* argv[] ) {
    unsigned int j;
    exp_enum exp_type;
    unsigned int num_tasks, num_objects, batch1, batch2;
    double elapsed;
    ...
    if( argc < 6 || exp_type == exp_NUM || num_objects <= 0 ) {
    ...
    object_t<int> param1[num_objects];
    object_t<int> param2[num_objects];
	...
	elapsed = experiment<int, indep>(
	    num_tasks, num_objects, batch1, batch2, param1, param2 );
	break;
    case exp_outdep:
