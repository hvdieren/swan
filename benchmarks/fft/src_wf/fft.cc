...
{
    if (i == i1 - 1) {
	leaf_call( fft_twiddle_gen1, in + i, out + i, W,
	           r, m, nW, nWdn * i, nWdn * m);
    } else {
	int i2 = (i + i1) / 2;
	spawn( fft_twiddle_gen, i, i2, in, out, W, nW, nWdn, r, m);
	spawn( fft_twiddle_gen, i2, i1, in, out, W, nW, nWdn, r, m);
	ssync();
    }
}
		    ...
		    COMPLEX * W, int nW)
{
    ...
    /* special cases */
    if (n == 32) {
	leaf_call(fft_base_32, in, out);
	return;
    }
    if (n == 16) {
	leaf_call(fft_base_16, in, out);
	return;
    }
    if (n == 8) {
	leaf_call(fft_base_8, in, out);
	return;
    }
    if (n == 4) {
	leaf_call(fft_base_4, in, out);
	return;
    }
    if (n == 2) {
	leaf_call(fft_base_2, in, out);
	return;
    }
    /* 
...
void cilk_fft(int n, COMPLEX * in, COMPLEX * out)
{
     ...
     */
    do {
	r = leaf_call(factor, l);
	*p++ = r;
	l /= r;
    } while (l > 1);

    ssync();			/* make sure W factors are computed */
...
void test_correctness(void)
{
	...
	/* fft-ize */
	call(cilk_fft, n, in1, out1);
	call(test_fft, n, in2, out2);
