			    ...
			    int nW, int nWdn, int r, int m)
{
     if (i == i1 - 1) {
	  ...
	  cilk_spawn fft_twiddle_gen(i2, i1, in, out, W, nW, nWdn, r, m);
     }
}
		    ...
		    COMPLEX * W, int nW)
{
     ...
     if (r < n) {
	   ...
	   */
	  if (r == 32)
	       cilk_spawn fft_unshuffle_32(0, m, in, out, m);
	  else if (r == 16)
	       cilk_spawn fft_unshuffle_16(0, m, in, out, m);
	  else if (r == 8)
	       cilk_spawn fft_unshuffle_8(0, m, in, out, m);
	  else if (r == 4)
	       cilk_spawn fft_unshuffle_4(0, m, in, out, m);
	  else if (r == 2)
	       cilk_spawn fft_unshuffle_2(0, m, in, out, m);
	  else
	       cilk_spawn unshuffle(0, m, in, out, r, m);
	  cilk_sync;
      ...
      */
     if (r == 2)
	  cilk_spawn fft_twiddle_2(0, m, in, out, W, nW, nW / n, m);
     else if (r == 4)
	  cilk_spawn fft_twiddle_4(0, m, in, out, W, nW, nW / n, m);
     else if (r == 8)
	  cilk_spawn fft_twiddle_8(0, m, in, out, W, nW, nW / n, m);
     else if (r == 16)
	  cilk_spawn fft_twiddle_16(0, m, in, out, W, nW, nW / n, m);
     else if (r == 32)
	  cilk_spawn fft_twiddle_32(0, m, in, out, W, nW, nW / n, m);
     else
	  cilk_spawn fft_twiddle_gen(0, m, in, out, W, nW, nW / n, r, m);
     cilk_sync;
...
void cilk_fft(int n, COMPLEX * in, COMPLEX * out)
{
     ...
     cilk_spawn fft_aux(n, in, out, factors, W, n);
     cilk_sync;
...
void test_correctness(void)
{
     ...
     for (n = 1; n < MAX; ++n) {
	  ...
	  /* fft-ize */
	  cilk_spawn cilk_fft(n, in1, out1);
	  cilk_sync;
	  cilk_spawn test_fft(n, in2, out2);
	  cilk_sync;
...
void test_speed(long size)
{
     ...
     cilk_spawn cilk_fft(size, in, out);
     cilk_sync;
...
int cilk_main(int argc, char *argv[])
{
     ...
     if (correctness)
	  cilk_spawn test_correctness();
     else
	  cilk_spawn test_speed(size);
